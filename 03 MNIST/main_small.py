...
class MNSIT:
    ...
    def __init__(self, data_dir="data/", rng=np.random.default_rng(seed=50)):
...
class tensorboar_handler:
    ...
    def __init__(self, log_dir="logs/"):
        ...
        self.model_summary = tf.summary.create_file_writer(
            self.log_dir + "/" + current_time + "/model_summary"
        )
    ...
    def close(self, summary):
        writer = self.model_summary
        with writer.as_default():
            tf.summary.text("model_summary", summary, step=0)
...
dropout = 0.1
reg = tf.keras.regularizers.L1L2(l1=0.0, l2=0.00)

model = tf.keras.models.Sequential()
model.add(tf.keras.layers.Input(shape=(28, 28, 1)))
model.add(tf.keras.layers.AveragePooling2D(pool_size=(5, 5)))
model.add(tf.keras.layers.Conv2D(3, (3, 3), activation="relu", kernel_regularizer=reg))
model.add(tf.keras.layers.Flatten())
model.add(tf.keras.layers.Dropout(dropout))
model.add(tf.keras.layers.Dense(5, activation="relu", kernel_regularizer=reg))
...
board = tensorboar_handler("logs_small/")

bar = trange(55000)
for epoch in bar:
    with tf.GradientTape() as tape:
        x, y = mnist.get_batch("train", 64)
